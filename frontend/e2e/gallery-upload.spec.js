...
test('gallery batch retains successes and retries only a failed photo', async ({
  page,
}) => {
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (path === '/api/process-uploaded-clothes') {
      ...
      if (uploads === 2) {
        await route.fulfill({
          status: 422,
          json: { detail: 'No garment found in second photo' },
        })
      } else {
        await route.fulfill({
          json: {
            garments: [
              { id: `garment-${uploads}`, front_url: '', category: 'top' },
            ],
          },
        })
      }
    ...
    await route.fulfill({
      json: { avatar_url: null, profile: null, garments: [] },
    })
  })
  ...
  await page.getByLabel('Clothing photos', { exact: true }).setInputFiles([
  ...
  await page.getByRole('button', { name: 'Add clothes' }).click()
  await expect(page.getByText('No garment found in second photo')).toBeVisible()
  await expect(page.getByText('Added 1 piece', { exact: true })).toHaveCount(1)
  await page.getByRole('button', { name: 'Retry trousers.png' }).click()
  await expect(page.getByText('Added 1 piece', { exact: true })).toHaveCount(2)
